...
if plot_output==1
    ...
    figure('units','normalized','outerposition',[0 0 1 1]) %opens new figure and makes it full screen
    clf
...
%     xlabel('Year')
    ...
    grid on
    grid minor
    title('\it Terminus position for defined min/max intervals')
...
%     xlabel('Year')
    ...
    grid on
    grid minor
    title('\it Distance change (m)')
    
    %Rate change

    ax2=subplot(4,3,10:11)
...
%     xlabel('Year')
    ...
    grid on
    grid minor
    title('\it Rate of change (m yr^-^1)')
    ...
    grid on
    grid minor
    title('\it Results from centreline')
    ...
    %adjust colormap to +/- 90th %ile of absolute values
    range1=prctile(abs(Results.DistanceChange(:)),90);
    range2=prctile(abs(Results.RateChange(:)),90);
    colormap(ax1,cmap)
    colormap(ax2,cmap)
    caxis(ax1,[-range1,range1])
    caxis(ax2,[-range2,range2])
