...
% If using terminus_format = 1, terminus_filename can be replaced by []
    ...
    if nansum(isnan(centreline.X))>0 
    ...
    if centreline.X(1)>-180 && centreline.X(1)<180 && centreline.Y(1)>-90 ...
        && centreline.Y(1)<90
        [~,~,zone]=ll2utm(centreline.Y(1),centreline.X(1),'wgs84');
        [centreline.X,centreline.Y]=ll2utm(centreline.Y(:),centreline.X(:),'wgs84',zone);
    else
        centreline.X=centreline.X';
        centreline.Y=centreline.Y';
    end
    ...
    elseif terminus_format==2
        ...
        %for n=1:length(termini_dummy(:,1))
            ...
            if strcmp(termini_dummy(n,1).Date(5),'-')
                ...
                termini_date(n,3)=str2num(termini_dummy(n,1).Date(7:8)); %day
            end
            ...
            if termini{n,1}.X(1)>-180 && termini{n,1}.X(1)<180 && termini{n,1}.Y(1)>-90 ...
                && termini{n,1}.Y(1)<90
                [termini{n,1}.X,termini{n,1}.Y]=ll2utm(termini{n,1}.Y(:),termini{n,1}.X(:),'wgs84',zone);
            else
                termini{n,1}.X=termini{n,1}.X';
                termini{n,1}.Y=termini{n,1}.Y';
            end
